...
# Clean data
df = df.loc[(df['value'] >= df['value'].quantile(0.025)) & (df['value'] <=         
...
def draw_bar_plot():
    ...
    # Draw bar plot
    fig, ty = plt.subplots()
    ty.set_title("Daily freeCodeCamp Forum Average Page Views per Month")

    chart = sns.barplot(data=df_bar, x="Years", y="Average Page Views", hue="Months",       
    palette="tab10")
